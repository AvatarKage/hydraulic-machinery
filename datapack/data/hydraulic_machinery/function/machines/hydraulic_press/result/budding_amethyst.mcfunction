...
execute \
    ...
    if score @s hydraulic_machinery.success_roll matches ..16 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_netherite] \
    if score @s hydraulic_machinery.success_roll matches ..32 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run setblock ~ ~-2 ~ minecraft:budding_amethyst

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run playsound block.amethyst_block.break block @a[distance=..12] \
        ~ ~-2 ~ 1 0.5

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run particle minecraft:block{block_state:"minecraft:budding_amethyst"} \
