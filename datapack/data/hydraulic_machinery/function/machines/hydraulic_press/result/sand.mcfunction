...
execute \
    ...
    if score @s hydraulic_machinery.success_roll matches ..5 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_iron] \
    if score @s hydraulic_machinery.success_roll matches ..15 \
    run tag @s add hydraulic_machinery_success
...
execute \
    ...
    if score @s hydraulic_machinery.success_roll matches ..30 \
    run tag @s add hydraulic_machinery_success
...
execute \
    ...
    run setblock ~ ~-2 ~ minecraft:sand

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run playsound block.sand.break block @a[distance=..12] \
        ~ ~-2 ~ 1 0.5

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run particle minecraft:block{block_state:"minecraft:sand"} \
