execute \
    ...
    if block ~ ~-2 ~ minecraft:dead_tube_coral_block \
    unless entity @e[\
        type=item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:dead_tube_coral_block"}}\
    ] \
    unless entity @e[\
        type=glow_item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:dead_tube_coral_block"}}\
    ] \
    run function hydraulic_machinery:machines/wet_press/result/tube_coral_block
