execute \
    if block ~ ~ ~ #hydraulic_machinery:solid_copper_blocks \
    align xyz positioned ~0.5 ~0.5 ~0.5 \
    unless entity @e[ \
        type=minecraft:armor_stand, \
        tag=hydraulic_machinery_hydraulic_marker, \
        tag=hydraulic_machinery_tier_copper, \
        distance=..0.5 \
    ] \
    run summon minecraft:armor_stand ~ ~ ~ \
        ...
        Tags:[ \
            ...
            "hydraulic_machinery_tier_copper" \
        ]}
...
execute if block ~ ~ ~ #hydraulic_machinery:solid_copper_blocks run return 0

scoreboard players add @s hydraulic_machinery.raycast 1

execute \
    if score @s hydraulic_machinery.raycast matches ..31 positioned ^ ^ ^0.25 \
    run function hydraulic_machinery:management/raycast/copper_blocks
