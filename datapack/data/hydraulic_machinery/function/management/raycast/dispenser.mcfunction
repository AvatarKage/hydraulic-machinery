execute \
    if block ~ ~ ~ minecraft:dispenser \
    align xyz positioned ~0.5 ~0.5 ~0.5 \
    unless entity @e[type=minecraft:armor_stand,tag=hydraulic_machinery_dispenser_marker,distance=..0.5] \
    run summon minecraft:armor_stand ~ ~ ~ \
        {Invulnerable:1b,NoGravity:1b,Invisible:1b,Marker:1b, \
        Tags:["hydraulic_machinery_marker","hydraulic_machinery_dispenser_marker"]}

execute if block ~ ~ ~ minecraft:dispenser run return 0
