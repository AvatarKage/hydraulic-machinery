execute \
    as @e[type=minecraft:armor_stand,tag=hydraulic_machinery_wheat_marker] \
    at @s \
    ...
    run function hydraulic_machinery:management/cleanup/wheat_press
...
execute \
    ...
    unless block ~ ~ ~ minecraft:hay_block \
    if block ~ ~ ~ #hydraulic_machinery:replaceable \
    unless block ~ ~-1 ~ minecraft:hay_block \
    run function hydraulic_machinery:management/cleanup/wheat_press
