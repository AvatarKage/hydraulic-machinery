...
execute \
    ...
    if score @s hydraulic_machinery.success_roll matches ..40 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_gold] \
    if score @s hydraulic_machinery.success_roll matches ..75 \
    run tag @s add hydraulic_machinery_success
...
execute \
    ...
    run setblock ~ ~-2 ~ minecraft:cobbled_deepslate

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run playsound block.deepslate.break block @a[distance=..12] \
        ~ ~-2 ~ 1 0.5

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run particle minecraft:block{block_state:"minecraft:cobbled_deepslate"} \
