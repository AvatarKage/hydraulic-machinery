...
execute \
    if entity @s[tag=hydraulic_machinery_tier_copper] \
    if score @s hydraulic_machinery.success_roll matches ..10 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_iron] \
    if score @s hydraulic_machinery.success_roll matches ..25 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_gold] \
    if score @s hydraulic_machinery.success_roll matches ..45 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_diamond] \
    if score @s hydraulic_machinery.success_roll matches ..70 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_tier_netherite] \
    if score @s hydraulic_machinery.success_roll matches ..100 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run setblock ~ ~-2 ~ minecraft:gravel

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run playsound block.gravel.break block @a[distance=..12] \
        ~ ~-2 ~ 1 0.5

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run particle minecraft:block{block_state:"minecraft:gravel"} \
