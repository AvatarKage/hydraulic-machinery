execute \
    ...
    if block ~ ~-2 ~ minecraft:prismarine_bricks \
    unless entity @e[\
        type=item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:prismarine_bricks"}}\
    ] \
    unless entity @e[\
        type=glow_item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:prismarine_bricks"}}\
    ] \
    run function hydraulic_machinery:machines/hydraulic_press/result/prismarine
