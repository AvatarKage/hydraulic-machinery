...
execute \
    if score @s hydraulic_machinery.success_roll matches ..35 \
    run tag @s add hydraulic_machinery_success

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run setblock ~ ~-2 ~ minecraft:clay

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run playsound entity.player.splash block @a[distance=..12] \
        ~ ~-2 ~ 1 0.5

execute \
    if entity @s[tag=hydraulic_machinery_success] \
    run particle minecraft:falling_water \
