execute \
    ...
    if block ~ ~-2 ~ minecraft:light_gray_concrete_powder \
    unless entity @e[\
        type=item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:light_gray_concrete_powder"}}\
    ] \
    unless entity @e[\
        type=glow_item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:light_gray_concrete_powder"}}\
    ] \
    run function hydraulic_machinery:machines/wet_press/result/concrete/light_gray_concrete
