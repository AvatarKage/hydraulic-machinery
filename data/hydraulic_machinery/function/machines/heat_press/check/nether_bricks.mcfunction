execute \
    ...
    if block ~ ~-2 ~ minecraft:nether_bricks \
    unless entity @e[\
        type=item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:nether_bricks"}}\
    ] \
    unless entity @e[\
        type=glow_item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:nether_bricks"}}\
    ] \
    run function hydraulic_machinery:machines/heat_press/result/cracked_nether_bricks
