execute \
    ...
    if block ~ ~-2 ~ minecraft:blue_terracotta \
    unless entity @e[\
        type=item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:blue_terracotta"}}\
    ] \
    unless entity @e[\
        type=glow_item_frame,limit=1,sort=nearest,dx=-1,dy=2,dz=-1,distance=..3, \
        nbt={Item:{id:"minecraft:blue_terracotta"}}\
    ] \
    run function hydraulic_machinery:machines/heat_press/result/glazed_terracotta/blue_glazed_terracotta
