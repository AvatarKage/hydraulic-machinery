execute \
    ...
    unless entity @e[type=minecraft:armor_stand,tag=hydraulic_machinery_stonecutter_marker,distance=..0.5] \
    run summon minecraft:armor_stand ~ ~ ~ \
        {Invulnerable:1b,NoGravity:1b,Invisible:1b,Marker:1b, \
        Tags:["hydraulic_machinery_marker","hydraulic_machinery_stonecutter_marker"]}

execute if block ~ ~ ~ minecraft:stonecutter run return 0
