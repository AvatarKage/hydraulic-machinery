execute \
    ...
    run summon minecraft:armor_stand ~ ~ ~ \
        ...
        Tags:[ \
            ...
            "hydraulic_machinery_wax_marker" \
        ]}
...
execute if block ~ ~ ~ minecraft:honeycomb_block run return 0

scoreboard players add @s hydraulic_machinery.raycast 1

execute \
    if score @s hydraulic_machinery.raycast matches ..31 positioned ^ ^ ^0.25 \
    run function hydraulic_machinery:management/raycast/honeycomb_block
